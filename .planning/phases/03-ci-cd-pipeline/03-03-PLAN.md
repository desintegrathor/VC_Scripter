...
<task type="auto">
  ...
  1. Baseline test function found in source
  2. Three YAML files generated
  3. YAML contains expected keys
  </verify>
  <done>Baseline test function verified present in test_validation.py, baseline YAML files generated for all 3 test cases, ready to commit</done>
</task>
...
3. Baseline test function verified: grep confirms function exists before running pytest
4. Baselines committed: Version-controlled in Git repository
5. Branch protection: "validate" status check required on main
6. Regression detection: Modified baseline triggers test failure
7. Merge blocking: Failed validation prevents PR merge
</verification>
...
- Baseline test function existence verified before pytest execution
