...
must_haves:
  ...
  artifacts:
    ...
    - path: "vcdecomp/tests/conftest.py"
      provides: "Pytest configuration for baseline directory"
      contains: "pytest_regressions_data_dir"
  key_links:
    ...
    - from: "vcdecomp/tests/conftest.py"
      to: ".planning/baselines/"
      via: "pytest_regressions_data_dir configuration"
      pattern: "\\.planning/baselines"
---
...
<task type="auto">
  ...
  <action>
  Create GitHub Actions workflow for automated validation on ALL push and pull request events (not limited to main branch):
  ...
  - `on: [push, pull_request]`: Triggers on ALL commits (satisfies VALID-05 "runs validation on all commits")
  ...
  2. Contains `on: [push, pull_request]` (NOT limited to specific branches)
  3. Contains `runs-on: [self-hosted, windows, x64]`
  4. Uses actions/upload-artifact@v4 (not v3)
  5. Has `continue-on-error: true` on pytest step
  6. Has `if: always()` on artifact upload
  </verify>
  <done>GitHub Actions workflow exists with self-hosted runner targeting, pytest execution, and artifact upload on all outcomes. Workflow triggers on ALL push and PR events without branch filters.</done>
</task>

<task type="auto">
  <name>Add Baseline Regression Testing</name>
  <files>vcdecomp/tests/test_validation.py, vcdecomp/tests/conftest.py, .planning/baselines/.gitkeep</files>
  <action>
  1. Configure pytest baseline directory in conftest.py:

  Add to `vcdecomp/tests/conftest.py`:
  ```python
  import pytest
  ...
  This fixture tells pytest-regressions to store baseline YAML files in `.planning/baselines/` instead of the default `tests/` directory, enabling version-controlled regression tracking (satisfies TEST-06).

  2. Extend test_validation.py to add a NEW baseline regression test:

  First, verify required imports are present at the top of test_validation.py:
  ```python
  from vcdecomp.validation.validator import ValidationOrchestrator, DifferenceCategory
  ```

  If `DifferenceCategory` is missing, add it to the imports line.

  Then add new test function `test_decompilation_validation_with_baseline` (this is a SEPARATE test, not replacing the existing one):
  ...
  3. Create baseline storage directory:
  ```bash
  mkdir -p .planning/baselines
  echo "# Pytest regression baselines" > .planning/baselines/.gitkeep
  ```

  NOTE: Baselines auto-generate on first test run (Plan 03-03 Task 3). The directory structure is created now, but baseline YAML files won't exist until first execution.
  ...
  grep -n "pytest_regressions_data_dir" vcdecomp/tests/conftest.py
  # Should return line number showing fixture exists

  # Verify DifferenceCategory import exists in test_validation.py
  grep "from vcdecomp.validation.validator import.*DifferenceCategory" vcdecomp/tests/test_validation.py
  # Should show import line with DifferenceCategory

  # Verify data_regression fixture is used
  grep "data_regression" vcdecomp/tests/test_validation.py
  # Should show test function signature and .check() call

  # Check test file has NEW baseline test (separate from existing test)
  grep -n "def test_decompilation_validation_with_baseline" vcdecomp/tests/test_validation.py

  # Verify BOTH test functions exist (original + baseline variant)
  grep -c "def test_decompilation_validation" vcdecomp/tests/test_validation.py
  # Should return 2 (test_decompilation_validation + test_decompilation_validation_with_baseline)
  ...
  Should show:
  - conftest.py contains pytest_regressions_data_dir fixture pointing to .planning/baselines/
  - test_validation.py imports DifferenceCategory
  - test_validation.py contains TWO test functions (original + baseline variant)
  - grep -c returns 2 (both functions present)
  - .planning/baselines/ directory exists (empty until first test run in Plan 03-03)
  </verify>
  <done>conftest.py configures baseline directory as .planning/baselines/ (satisfies TEST-06), test_validation.py imports DifferenceCategory and includes NEW baseline regression test alongside existing test (verified by grep -c returning 2), .planning/baselines/ directory created for storing YAML baselines (files generated in Plan 03-03)</done>
</task>
...
3. Workflow triggers: on: [push, pull_request] without branch filters (runs on ALL commits)
4. Baseline directory configured: conftest.py has pytest_regressions_data_dir fixture (TEST-06)
5. Imports verified: test_validation.py imports DifferenceCategory
6. Baseline test added: test_validation.py has NEW data_regression test variant (coexists with original, verified by grep -c)
7. Artifact upload configured: Workflow uploads JUnit XML and JSON reports
8. Continue-on-error set: Tests always upload artifacts even on failure
9. Baseline directory created: .planning/baselines/ ready for YAML files (generated in Plan 03-03)
</verification>
...
- Workflow triggers on ALL commits: `on: [push, pull_request]` without branch filters (satisfies VALID-05)
...
- conftest.py configures pytest_regressions_data_dir to point to .planning/baselines/ (satisfies TEST-06 - CRITICAL)
- test_validation.py imports DifferenceCategory from validation.validator
- NEW baseline regression test added to test_validation.py using data_regression fixture (coexists with existing test)
- Both test functions verified present: grep -c "def test_decompilation_validation" returns 2
- Baseline storage directory created at .planning/baselines/ (YAML files generated in Plan 03-03)
</success_criteria>
