...
<task type="auto">
  ...
  <action>
  Extend test_validation.py to add a NEW baseline regression test that coexists with the existing `test_decompilation_validation` test:

  1. Add new test function `test_decompilation_validation_with_baseline` (this is a SEPARATE test, not replacing the existing one):
  ...
  <done>test_validation.py includes NEW baseline regression test alongside existing test (verified by grep -c returning 2), .planning/baselines/ directory created for storing YAML baselines (files generated in Plan 03-03)</done>
</task>
...
4. Baseline test added: test_validation.py has NEW data_regression test variant (coexists with original, verified by grep -c)
5. Artifact upload configured: Workflow uploads JUnit XML and JSON reports
6. Continue-on-error set: Tests always upload artifacts even on failure
7. Baseline directory created: .planning/baselines/ ready for YAML files (generated in Plan 03-03)
</verification>
