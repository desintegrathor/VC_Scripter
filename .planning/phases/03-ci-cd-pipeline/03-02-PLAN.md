...
<task type="auto">
  ...
  # Should show BOTH tests exist
  grep -n "def test_decompilation_validation" vcdecomp/tests/test_validation.py | wc -l
  # Should return 2 (original test + baseline test)
  ...
  <done>test_validation.py includes NEW baseline regression test alongside existing test, .planning/baselines/ directory created for storing YAML baselines (files generated in Plan 03-03)</done>
</task>
...
3. Baseline test added: test_validation.py has NEW data_regression test variant (coexists with original)
...
6. Baseline directory created: .planning/baselines/ ready for YAML files (generated in Plan 03-03)
</verification>
