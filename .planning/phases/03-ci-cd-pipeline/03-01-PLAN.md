...
# NOTE: autonomous: false with 2 human checkpoints is EXPECTED for one-time runner setup
# This is a one-time infrastructure configuration task that requires:
# 1. Generating GitHub registration token (web UI only, expires in 1 hour)
# 2. Verifying runner is online before proceeding to automation (Plan 03-02)
# After this setup, all subsequent CI runs are fully automated with no human interaction
...
<task type="auto">
  ...
  1. Extract GitHub username from git remote:
     ...
     # Parse username from URL (format: https://github.com/USERNAME/REPO.git or git@github.com:USERNAME/REPO.git)
     ```
  ...
  2. Run configuration command:
     ...
     ```
     - Replace [username] with extracted GitHub username
