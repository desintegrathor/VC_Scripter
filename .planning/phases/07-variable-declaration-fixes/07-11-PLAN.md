...
echo "=== Code-level verification ===" > .test_artifacts_07-11/code_verification.txt

# Confirm RET analysis was changed
grep -A 5 "elif instr.mnemonic == \"RET\"" vcdecomp/core/ir/function_signature.py >> .test_artifacts_07-11/code_verification.txt

# Check that both branches NO LONGER both set to "int"
if grep -A 5 "elif instr.mnemonic == \"RET\"" vcdecomp/core/ir/function_signature.py | grep -q "void"; then
    echo "PASS: Code now sets void return type for RET arg=0" >> .test_artifacts_07-11/code_verification.txt
else
    echo "FAIL: Code still defaults all functions to int" >> .test_artifacts_07-11/code_verification.txt
fi

cat .test_artifacts_07-11/code_verification.txt
...
# Decompile to verify actual function declarations
...
echo "" >> .test_artifacts_07-11/code_verification.txt
echo "=== Decompiled function declarations ===" >> .test_artifacts_07-11/code_verification.txt
grep "func_0050" .test_artifacts_07-11/test1_fixed_returns.c | grep "^[a-z]" | head -1 >> .test_artifacts_07-11/code_verification.txt
grep "func_0119" .test_artifacts_07-11/test1_fixed_returns.c | grep "^[a-z]" | head -1 >> .test_artifacts_07-11/code_verification.txt
grep "func_0155" .test_artifacts_07-11/test1_fixed_returns.c | grep "^[a-z]" | head -1 >> .test_artifacts_07-11/code_verification.txt
...
- code_verification.txt shows actual code changes (grep confirms void return type logic added)
...
- test1_fixed_returns.c shows func_0050, func_0119, func_0155 declared as int (not void)
  </done>
</task>

<task type="auto">
  <name>Task 2: Validate Category 3 Elimination</name>
...
**Note:** Orchestrator already uses get_function_signature_string() which propagates return_type correctly (verified in context reading). Task 1 fix automatically propagates through:
1. detect_function_signature() sets sig.return_type (Task 1)
2. sig.to_c_signature() includes return_type in output
3. get_function_signature_string() returns to_c_signature() output
4. orchestrator uses signature directly in function header

No orchestrator changes needed - wiring already correct.
...
- Orchestrator wiring confirmed correct (no changes needed)
  </done>
</task>
