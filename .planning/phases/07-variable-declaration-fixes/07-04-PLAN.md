...
must_haves:
  ...
  artifacts:
    ...
    - path: "vcdecomp/core/ir/structure/analysis/value_trace.py"
...
<task type="auto">
  ...
  <files>vcdecomp/core/ir/structure/analysis/value_trace.py</files>
  <action>
Add loop bound analysis to existing value_trace.py module:

1. Verify value_trace.py exists in vcdecomp/core/ir/structure/analysis/ (confirmed via ls command)
2. Add method to ValueTracer class: trace_loop_bounds(ssa_func: SSAFunction) → Dict[var_name, BoundInfo]
   ```python
   @dataclass
   class BoundInfo:
...
5. Handle nested loops for multi-dimensional detection:
   ```python
...
Why this approach: Loop bounds are the strongest evidence for array dimensions. Conservative estimates with TODO comments prevent crashes while flagging uncertainty. value_trace.py exists and is the correct location for this analysis.
...
Log shows loop bound detection for arrays with min/max/step values.
  ...
  <done>
Loop bound analysis implemented in value_trace.py with confidence scoring. Test validation log shows array dimension inference from loop patterns.
  </done>
</task>
