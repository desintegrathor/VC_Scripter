...
must_haves:
  ...
  key_links:
    ...
    - from: "cfg.py entry_block detection"
      to: "ScriptMain function header"
      via: "Fixed entry point offset calculation"
      pattern: "entry_block.*=.*self\\.blocks"
---
...
1. **Negative offset indicates special entry point:**
   - Entry point offset from .scr header is -1098
   - This is relative to data segment, NOT code segment
   - CFG construction fails to find corresponding code address

2. **Entry block detection logic (likely in cfg.py):**
   - build_cfg() takes entry_offset from header
   - Must convert offset to instruction address
   - Entry block should be first block in CFG for ScriptMain

**Fix strategy:**

1. **Locate entry block detection in cfg.py:**
```python
# Search for entry point handling
grep -n "entry" vcdecomp/core/ir/cfg.py
```

2. **Expected fix (entry point offset conversion):**
```python
# PROBLEM: Direct offset comparison fails
entry_block = None
for block in blocks:
    if block.start_addr == entry_offset:
        entry_block = block
        break

# FIX: Handle header entry offset correctly
# Entry offset from header may be relative to different base
# For ScriptMain, entry is typically first instruction (address 0 in code segment)
if func_name == "ScriptMain" or entry_offset < 0:
    # ScriptMain entry is the function start
    entry_block = blocks[0] if blocks else None
else:
    # Regular functions: find block by offset
    for block in blocks:
        if block.start_addr == entry_offset:
            entry_block = block
            break
...
3. **Alternative: Check orchestrator.py function detection:**
- ScriptMain is special entry point function
- May need explicit handling in structured_decompile()
- Search for "ScriptMain" in orchestrator.py

4. **Validate fix:**
```python
# Entry block must exist for all functions
if not entry_block:
    logger.error(f"Entry block not found for {func_name} at offset {entry_offset}")
    # Fallback: use first block
    entry_block = blocks[0] if blocks else None
```

**Why this works:**
- ScriptMain has special header offset encoding (negative)
- Engine always calls ScriptMain first (entry point by definition)
- Entry block should be first block in CFG (address 0)
  </action>
  <verify>
NOTE: Requires Git Bash (bash.exe) available in PATH.
...
- Entry block detection handles negative offsets correctly (special engine entry points)
  </done>
</task>
