...
must_haves:
  ...
  artifacts:
    ...
    - path: "vcdecomp/core/ir/cfg.py"
      provides: "ScriptMain entry block detection verified"
      min_lines: 200
      exports: ["build_cfg"]
...
echo "=== Code-level verification ===" > .test_artifacts_07-10/code_verification.txt

# Confirm logger.warning was changed to logger.debug
grep -n "logger.debug.*orphaned" vcdecomp/core/ir/structure/orchestrator.py >> .test_artifacts_07-10/code_verification.txt
if [ $? -eq 0 ]; then
    echo "PASS: logger.warning changed to logger.debug" >> .test_artifacts_07-10/code_verification.txt
else
    echo "FAIL: logger.debug not found in expected location" >> .test_artifacts_07-10/code_verification.txt
fi

# Verify no logger.warning remains in emission path (around line 381)
grep -n "logger.warning" vcdecomp/core/ir/structure/orchestrator.py | grep -E "(381|orphan)" >> .test_artifacts_07-10/code_verification.txt || echo "PASS: No logger.warning at line 381" >> .test_artifacts_07-10/code_verification.txt

cat .test_artifacts_07-10/code_verification.txt
...
- code_verification.txt shows logger.debug confirmed at expected location
...
<task type="auto">
  <name>Task 2: Diagnose and Fix ScriptMain Entry Block Detection</name>
  <files>vcdecomp/core/ir/cfg.py, vcdecomp/core/ir/structure/orchestrator.py</files>
  <action>
Diagnose WHERE "entry block not found" message originates, then fix ScriptMain function reconstruction.
...
1. **Find where error message is generated:**
```bash
# Locate the exact source of "entry block not found" error
grep -rn "entry block not found" vcdecomp/
```

2. **Check if cfg.py or orchestrator.py generates this message:**
Expected locations:
- orchestrator.py: Function detection/rendering code
- cfg.py: Entry point resolution code

3. **Understand current state:**
```bash
# Verify cfg.py _resolve_entry_ip() implementation
grep -A 20 "_resolve_entry_ip" vcdecomp/core/ir/cfg.py

# Check how orchestrator uses cfg.entry_block
grep -n "cfg.entry_block" vcdecomp/core/ir/structure/orchestrator.py
```

**AFTER DIAGNOSIS, APPLY FIX:**

**Scenario A: If orchestrator.py generates the error**
- Issue: orchestrator doesn't use cfg.entry_block properly
- Fix: Use cfg.entry_block from build_cfg() instead of manual lookup
```python
...
for block in blocks:
    if block.start == entry_offset:
        ...
        break
if not entry_block:
    print(f"// entry block not found")

# AFTER:
entry_block = cfg.entry_block  # Already resolved by _resolve_entry_ip()
```

**Scenario B: If cfg.py entry_block = 0 fallback fails**
- Issue: cfg.py line 158: `entry_block = start_to_block.get(entry_ip, 0)` may fail if block 0 doesn't exist
- Fix: Verify block 0 exists before fallback
```python
# Line 158 in cfg.py
entry_block = start_to_block.get(entry_ip)
if entry_block is None:
    # Fallback to first block if entry_ip not found
    entry_block = min(blocks.keys()) if blocks else 0
```

**Why diagnostic-first:**
- Current task action ASSUMES orchestrator.py is the problem
- Without confirming actual bug location, fix may not address root cause
- Diagnostic grep confirms WHERE to apply fix
...
- Proper wiring eliminates "entry block not found" error
  </action>
  <verify>
NOTE: Requires Git Bash (bash.exe) available in PATH.
...
# VERIFY CODE CHANGES (diagnostic + fix)
echo "=== Diagnostic Results ===" > .test_artifacts_07-10/scriptmain_verification.txt

# Show where error message was found
grep -rn "entry block not found" vcdecomp/ >> .test_artifacts_07-10/scriptmain_verification.txt

echo "" >> .test_artifacts_07-10/scriptmain_verification.txt
echo "=== Code Fix Verification ===" >> .test_artifacts_07-10/scriptmain_verification.txt

# Verify fix was applied (check for cfg.entry_block usage or improved fallback)
grep -A 3 "entry_block.*=.*cfg" vcdecomp/core/ir/structure/orchestrator.py >> .test_artifacts_07-10/scriptmain_verification.txt || \
grep -A 3 "entry_block.*=.*start_to_block" vcdecomp/core/ir/cfg.py >> .test_artifacts_07-10/scriptmain_verification.txt

cat .test_artifacts_07-10/scriptmain_verification.txt
...
```
  ...
  <done>
- scriptmain_verification.txt shows where "entry block not found" was located
- Code fix verified via grep showing actual implementation change
