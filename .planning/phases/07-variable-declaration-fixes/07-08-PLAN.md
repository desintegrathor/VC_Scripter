...
must_haves:
  ...
  artifacts:
    ...
    - path: ".test_artifacts_07-08/test1_final.c"
      provides: "Test1 decompiled output with fixed types"
      min_lines: 100
    - path: ".test_artifacts_07-08/test2_final.c"
      provides: "Test2 decompiled output with fixed types"
      min_lines: 100
    - path: ".test_artifacts_07-08/test3_final.c"
      provides: "Test3 decompiled output with fixed types"
      min_lines: 100
  key_links:
...
- Field access patterns only used when NO opcode evidence exists
  ...
  <verify>
NOTE: Requires Git Bash (bash.exe) available in PATH. Windows Command Prompt not supported.
...
<task type="auto">
  <name>Task 2: Disable Field Access Inference for Opcode Variables</name>
  <files>vcdecomp/core/ir/structure/analysis/variables.py</files>
  <action>
Comment out _struct_ranges field access inference to prevent false positives from overriding opcode types.
...
**Fix:** Comment out the entire _struct_ranges section to disable field access inference:
...
**Rationale:**
- Task 1 already establishes opcode-first priority, which prevents most false positives
- Commenting out _struct_ranges completely eliminates any remaining edge cases
- Confidence-scored struct inference from struct_type_map (07-07) is more reliable
- Can re-enable with confidence threshold in Phase 8 if needed

**Why aggressive fix:** Opcode evidence is ALWAYS more reliable than field access patterns. Field access can be false positives from array indexing. When in doubt, prefer primitive types.
  </action>
  <verify>
...
```
  ...
  <done>
- _struct_ranges section commented out in variables.py
...
4. _struct_ranges inference disabled entirely
...
3. _struct_ranges field access inference disabled
