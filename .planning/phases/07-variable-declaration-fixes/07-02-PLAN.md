...
Log shows type inference invoked for each function before variable collection.
  ...
  <done>
Orchestrator invokes type_inference.integrate_with_ssa_values() after SSA construction and before variable collection. Log confirms execution for each function.
  </done>
</task>
...
1. Update process_value() type resolution priority:
   ...
   ```
2. Add type mapping from opcodes.ResultType to C types:
   - ResultType.FLOAT → "float"
   - ResultType.INT → "int"
   - ResultType.DOUBLE → "double"
   - ResultType.CHAR → "char"
   - ResultType.SHORT → "short"
   - ResultType.UNKNOWN → default_type (fallback)
3. Insert check after line 78 (struct types):
   ...
   ```
4. Add helper function: result_type_to_c_type(result_type: ResultType) → str
5. Preserve existing struct inference logic (lines 112-163) - struct types override opcode types

Why this approach: Minimal change to existing priority system. SSA refined types slot in between high-confidence struct inference and low-confidence defaults. Preserves all Phase 6 fixes.
...
- Struct types preserved from function call inference
  ...
  <done>
Variable declarations generated from SSA refined types. Manual inspection shows float/int/struct declarations matching actual usage patterns from ERROR_BASELINE.md Pattern 2 examples.
  </done>
</task>
