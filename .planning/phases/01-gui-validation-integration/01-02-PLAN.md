...
files_modified:
  ...
  - vcdecomp/gui/dialogs/validation_settings.py
autonomous: false
...
must_haves:
  ...
  artifacts:
    - path: vcdecomp/gui/main_window.py
      provides: Settings menu integration with QSettings persistence
      min_lines: 450
      contains: "QSettings"
    - path: vcdecomp/gui/dialogs/validation_settings.py
      provides: ValidationSettingsDialog
      exists: true
  key_links:
    ...
    - from: vcdecomp/gui/main_window.py
      to: QSettings
      via: Load/save compiler_dir
      pattern: "QSettings.*compiler_dir"
---

## Objective

Integrate validation settings into main menu and persist configuration using QSettings. Verify end-to-end validation flow works correctly for test scripts.
...
3. Implement show_validation_settings() method:
   ...
   def show_validation_settings(self):
       ...
       settings = QSettings("VC-Scripter", "Decompiler")
       compiler_dir = settings.value("validation/compiler_dir", "")
       include_dirs = settings.value("validation/include_dirs", [])
       timeout = settings.value("validation/timeout", 30, type=int)

       # Set dialog values
       dialog.set_compiler_dir(compiler_dir)
       if include_dirs:
           dialog.set_include_dirs(include_dirs)
       dialog.set_timeout(timeout)

       # Show dialog and save if accepted
       if dialog.exec() == QDialog.DialogCode.Accepted:
           settings.setValue("validation/compiler_dir", dialog.get_compiler_dir())
           settings.setValue("validation/include_dirs", dialog.get_include_dirs())
           settings.setValue("validation/timeout", dialog.get_timeout())

           # Update ValidationPanel with new settings
           self.validation_panel.compiler_dir = dialog.get_compiler_dir()
           self.validation_panel.include_dirs = dialog.get_include_dirs()
           self.validation_panel.timeout = dialog.get_timeout()

           self.statusBar().showMessage("Validation settings saved", 3000)
   ```

4. Import QSettings and QDialog at top:
   ```python
   from PyQt6.QtCore import Qt, QRegularExpression, QSettings
   from PyQt6.QtWidgets import (..., QDialog)
   ```

5. In __init__ method, after self.init_ui(), load saved settings and apply to validation_panel:
   ```python
   ...
   # Apply to validation panel
   if compiler_dir:
       self.validation_panel.compiler_dir = compiler_dir
   if include_dirs:
       self.validation_panel.include_dirs = include_dirs
   self.validation_panel.timeout = timeout
   ```

Why this approach:
- QSettings provides platform-native settings storage (registry on Windows, plist on macOS)
- "VC-Scripter/Decompiler" namespace avoids conflicts with other apps
- Loading settings in __init__ ensures ValidationPanel has correct config on startup
- Ctrl+, is standard settings shortcut across many applications
- Saving immediately on dialog accept prevents loss of configuration
  </action>
  <verify>
...
- ValidationPanel receives updated settings when dialog is accepted
...
1. In validate_current_script() method, after checking self.scr is not None, add settings check:
   ...
   if not self.validation_panel.compiler_dir or not Path(self.validation_panel.compiler_dir).exists():
...
2. Import Path from pathlib (already imported for temp file creation)
...
- Matches pattern from existing ValidationPanel.on_validate_clicked (lines 406-414 in validation_view.py)
  </action>
  <verify>
1. Run GUI: `python -m vcdecomp gui` (fresh start, no settings saved)
...
7. Validation should proceed automatically
  ...
  <done>
- validate_current_script() checks for configured compiler directory
...
- QSettings persistence for compiler directory, include directories, and timeout
- Settings loaded on GUI startup and applied to ValidationPanel
...
1. **Fresh start (clear settings):**
   ```bash
   # On Windows, delete registry key (optional - skip if unsure):
   # reg delete "HKCU\Software\VC-Scripter\Decompiler" /f
...
1. **Settings integration:**
   ...
   - QSettings persists compiler_dir, include_dirs, timeout
   - Settings load on GUI startup
...
- [ ] Settings persist between GUI sessions using QSettings
- [ ] ValidationPanel receives settings on startup
...
- Settings integration approach (QSettings persistence)
