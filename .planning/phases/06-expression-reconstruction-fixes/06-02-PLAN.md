...
**IMPORTANT SCOPE CONSTRAINT:** Fix top 3-5 patterns ONLY. Defer additional patterns to future work.

For each top-priority pattern (3-5 patterns maximum):
...
**STOP after 3-5 patterns** - Do not attempt to fix all errors in one pass. Document remaining patterns for future work.
...
- Fix 3-5 patterns maximum, then stop
...
5. Document in SUMMARY if pattern needs deeper investigation
  ...
  <verify>
```bash
# Verify code files were modified
git diff --name-only vcdecomp/core/ir/expr.py vcdecomp/core/ir/parenthesization.py
git diff vcdecomp/core/ir/expr.py | head -50
git diff vcdecomp/core/ir/parenthesization.py | head -50

# Verify needs_parens() wiring if parenthesization was modified
if git diff vcdecomp/core/ir/parenthesization.py | grep -q "def needs_parens"; then
  echo "Checking needs_parens() calls include parent_operator parameter..."
  grep -n "needs_parens.*parent_operator" vcdecomp/core/ir/expr.py || echo "WARNING: parent_operator parameter missing in some needs_parens() calls"
fi
...
- Git diff confirms expr.py or parenthesization.py modified
- needs_parens() calls verified to include parent_operator parameter (if modified)
  </done>
</task>
