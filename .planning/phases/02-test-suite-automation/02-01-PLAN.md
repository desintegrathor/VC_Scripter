...
must_haves:
  truths:
    ...
    - "Each test script runs as an individual pytest test item"
  artifacts:
...
1. **Decompile step** - Use existing decompilation pipeline:
   - Load SCRFile from scr_path
   - Build SSA: `build_ssa_all_blocks(scr)`
   - Get function boundaries from Disassembler
   - Generate includes: `generate_include_block(scr)`
   - Analyze globals: `GlobalResolver(ssa_func, aggressive_typing=True, infer_structs=False)`
   - Decompile all functions: `format_structured_function_named()` for each function
   - Write to tmp_path / f"{test_id}_decompiled.c"

2. **Validate step** - Call ValidationOrchestrator:
   - `result = validation_orchestrator.validate(scr_path, decompiled_path)`

3. **Report step** - Print detailed results:
   - Print separator line and test ID
   - Print full ValidationResult (has __str__ method)
   - Show compilation errors if any: `result.compilation_result.errors`
   - Show bytecode differences if any: `result.difference_summary`
...
2. Compare decompiled output structure to original .c file
3. Note patterns in failures (expression reconstruction, variable declarations, control flow)
...
✓ Bytecode differences are categorized (semantic vs cosmetic)
...
5. **Requirements satisfied:**
   ...
   - TEST-04: ✓ Test suite categorizes failures by error type (via ValidationResult)
