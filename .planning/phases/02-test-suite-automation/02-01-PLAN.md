...
**Test corpus discovery (CASE-SENSITIVE PATHS - verified on filesystem):**
```python
TEST_SCRIPTS = [
...
**Note on case sensitivity:** test1 and test2 use lowercase filenames (tt.scr, tdm.c), test3 uses UPPERCASE (LEVEL.SCR, LEVEL.C). This matches the exact filesystem layout.
...
- Import DifferenceCategory from vcdecomp.validation.difference_types for categorization (VERIFIED: import path is correct, field exists in ValidationResult)
- Import pytest for parametrization
- Import Path from pathlib for cross-platform paths

**Verification notes from checker investigation:**
- ✓ Import path `from vcdecomp.validation.difference_types import DifferenceCategory` is CORRECT (verified in codebase)
- ✓ ValidationResult.categorized_differences field EXISTS (validation_types.py:42-43)
- ✓ Globals handling is CORRECT: decompiler outputs inline references (e.g., `data[322]`), not separate declarations
