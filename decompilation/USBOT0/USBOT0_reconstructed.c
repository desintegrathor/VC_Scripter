...
float buddy_follow_distance = 8.0f;     // Jak blízko následovat partnera (metry)
...
int DoBuddyBehavior(dword player, float elapsed) {
    ...
    // Leader se chová normálně, ale nesmí kempit (nebo oznámí buddymu)
    if (buddy_role == BUDDY_ROLE_LEADER) {
        // Pokud začnu kempit, oznámím to buddymu
        // (toto se řeší v camping systému)
    ...
    if (buddy_role == BUDDY_ROLE_BUDDY) {
        ...
        // (chovej se normálně, ale zůstaň poblíž)
        return 0;
    }
...
void MainAILoop(s_SC_P_info *info) {
    ...
    // NOVÉ: Vlajkonoš - běž přímo domů, žádné zdržování!
    if (am_flag_carrier) {
        SC_P_Ai_SetMoveMode(pl_id, SC_P_AI_MOVEMODE_RUN);
        SC_P_Ai_SetBattleMode(pl_id, SC_P_AI_BATTLEMODE_ATTACKAGR);
