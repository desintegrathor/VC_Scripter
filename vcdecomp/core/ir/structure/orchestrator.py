...
def format_structured_function_named(ssa_func: SSAFunction, func_name: str, entry_addr: int, end_addr: int = None, function_bounds=None, style: str = "normal", heritage_metadata: Optional[Dict] = None) -> str:
    ...
    from ..local_type_tracker import LocalVariableTypeTracker
    type_tracker = LocalVariableTypeTracker(
        ssa_func=ssa_func,
    ...
    type_tracker.analyze_ssa_patterns(func_block_ids)
    type_tracker.import_field_tracker_results()
    formatter.set_type_tracker(type_tracker)
    ...
    # Pass type_tracker for unified type resolution (coordinates declarations with usage)
    #
    # UNIFIED TYPE TRACKER: Finalize type tracker before variable collection
    # Note: In the current architecture, declarations are collected before main formatting.
    # The type_tracker's SSA pattern analysis (Pass 1) and field_tracker import have already run,
