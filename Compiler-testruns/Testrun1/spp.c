...
void UpdateSideFrags(void){
	SC_sgi(500,gSideFrags[0]);
	SC_sgi(501,gSideFrags[1]);
}
...
int ScriptMain(s_SC_NET_info *info){
	...
	switch(info->message){
		...
		case 4:
			...
			gCLN_SideFrags[0] = SC_ggi(500);
			gCLN_SideFrags[1] = SC_ggi(501);
		...
		case 1:
			...
			hudinfo.sort_by[0] = 2;
			hudinfo.sort_by[1] = 3;
			hudinfo.sort_by[2] = 4 | 0x80000000;
			hudinfo.sort_by[3] = 5 | 0x80000000;

			hudinfo.pl_mask = 0x04 | 0x08 | 0x10;
			hudinfo.use_sides = 1;
		...
		case 10:
			...
			SC_ZeroMem(&gSideFrags,sizeof(gSideFrags));
			UpdateSideFrags();
